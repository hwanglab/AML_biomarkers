...
FindClusterFreqDF(stem_paired, c("patient_id", "timepoint"), "clusters") %>%
  ...
  summarise(pval = wilcox.test(freq ~ timepoint)$p.value,
            .groups = "keep") %>%
  arrange(pval)
...
ggsurvplot(data = sc_survival_data,
           fit = sc_survival, 
           xlab = "Days", 
...
ann <- getBM(attributes = c("ensembl_gene_id_version",
                            "hgnc_symbol"),
             mart = ensembl)
...
ann <- getBM(attributes = c("ensembl_gene_id",
                            "hgnc_symbol"),
             mart = ensembl)

plan("multisession")
res <- files %>% future_map( ~ dplyr::select(., 1:2) %>%
...
ggsurvplot(data = target_survival_data_itd,
...
ggsurvplot(data = target_survival_data_no_itd,
...
lasso_data <- cibersort %>%
  mutate(across(where(is_character), str_to_lower)) %>%
  filter(`FLT3/ITD positive?` == "yes") %>%
  dplyr::select(cluster13, `WBC at Diagnosis`, `Bone marrow leukemic blast percentage (%)`,
...
score <- cibersort %>%
  ...
  remove_missing() %>%
  set_names(c("cluster13", "wbc", "leukemic_blasts", "peripheral_blasts",
              "cyto_complex", "allelic_ratio", "efs_days", "status")) %>%
  mutate(score = -1127.73 * cluster13 - 1.14 * wbc + 2.79 * leukemic_blasts + 6.40 * peripheral_blasts - 94.19 * cyto_complex + 289.49,
