...
cibersort_output = "cibersort_results"
...
s_mode_mark = "{}/{}/s_mode".format(output_path, cibersort_output)
b_mode_mark = "{}/{}/b_mode".format(output_path, cibersort_output)
no_bc_mark = "{}/{}/no_cibersort_batch_correction".format(output_path, cibersort_output)

if argv.B_mode:
    if os.path.isfile(s_mode_mark):
        logger.critical("S-mode batch correction previously run")
        quit(1)
    if os.path.isfile(no_bc_mark):
        logger.critical("No batch correction previously run")
        quit(1)
elif argv.S_mode:
    if os.path.isfile(b_mode_mark):
        logger.critical("B-mode batch correction previously run")
        quit(1)
    if os.path.isfile(no_bc_mark):
        logger.critical("No batch correction previously run")
        quit(1)
...
if argv.B_mode:
    # Print file
    f = open(b_mode_mark, mode = "w")
    f.close()
elif argv.S_mode:
    f = open("s_mode")
    f.close()
else:
    f = open("no_cibersort_batch_correction")
    f.close()
